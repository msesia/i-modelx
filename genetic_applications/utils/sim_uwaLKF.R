...
# load swapped genoytpes in chr 21
# they are in a different order, but it does not matter because we know which ones are nonzero
...
for(b in 1:B) {
  ...
  # randomly assign each non-zero snps to global or local effect
  ...
  rm(list=setdiff(ls(), c("y", "covariates",  "map.real.chr21.orig.main", "map.real.chr21.main", "map.real.chr21", "map.real.chr21.orig",  "map.real",  "map.real.main", 
                          ...
                          "power_cond_association_deconstruct_main", "power_cond_association_deconstruct_int")))
...
results.df = data.frame(fdp = fdp, 
                        ...
                        sparsity = sparsity, amp = amp, fdr = fdr, propmain = propmain, method = "uwaLKF")
...
# Save fdp 
stats.path = paste0("UKB_stats/simint_paper_sskf_lasso_",
