...
# Class: R script
#
# Simulate performance of naive method
...
for(b in 1:B) {
  ...
  # randomly assign each non-zero snps to global or local effects
  ...
  ######################### load real genotypes: on the same dataset: search for interactions and test  #################################
  ...
  lasso.fit <- big_spLinReg(G.real.chr21, y=y, dfmax=dfmax, ncores=ncores, 
  ...
  G.real.chr21.nonzero = as_FBM(G.real.chr21[, index.nonzero.snps],
                                      backingfile = tempfile(tmpdir = "tmp"))
  
  # create interactions
  if(num_covariates == 1) {
    G.real.chr21.nonzero.int = matrix(NA, nrow = nrow(G.real.chr21.nonzero), ncol = ncol(G.real.chr21.nonzero)*3)
  }
  
  if(num_covariates == 2) {
    G.real.chr21.nonzero.int = matrix(NA, nrow = nrow(G.real.chr21.nonzero), ncol = ncol(G.real.chr21.nonzero)*5)
  }
  
  G.real.chr21.nonzero.int[, 1:ncol(G.real.chr21.nonzero)] = G.real.chr21.nonzero[]
  
  G.real.chr21.nonzero.int[, (ncol(G.real.chr21.nonzero)+1):(2*ncol(G.real.chr21.nonzero))] = G.real.chr21.nonzero[]*covariates[, 1]
  G.real.chr21.nonzero.int[, (ncol(G.real.chr21.nonzero)*2+1):(3*ncol(G.real.chr21.nonzero))] = G.real.chr21.nonzero[]*(1-covariates[, 1])
  
  
  if(num_covariates == 2) {
    G.real.chr21.nonzero.int[, (ncol(G.real.chr21.nonzero)*3+1):(4*ncol(G.real.chr21.nonzero))] = G.real.chr21.nonzero[]*covariates[, 2]
    G.real.chr21.nonzero.int[, (ncol(G.real.chr21.nonzero)*4+1):(5*ncol(G.real.chr21.nonzero))] = G.real.chr21.nonzero[]*(1-covariates[, 2])
  }
  
  
  G.real.chr21.nonzero.int = as_FBM(G.real.chr21.nonzero.int,
                                          backingfile = tempfile(tmpdir = "tmp"))
  ...
  G.scale.inter <- scaler.inter(G.real.chr21.nonzero.int)
  scaling.factors.inter <- G.scale.inter$scale
  cat("done.\n")
  
  # Run lasso on swapped genotypes for pre-screening 
  cat(sprintf("Fitting sparse linear regression with %d observations, %d variants and covariates... ",
              length(y), ncol(G.real.chr21.nonzero.int)))
  lasso.fit.inter <- big_spLinReg(G.real.chr21.nonzero.int, y=y, dfmax=dfmax, ncores=ncores, 
                                  covar.train=covariates, pf.covar = rep(0, ncol(covariates)))
  ...
  # choose top interactions using knockoff importance statistics (difference! of abs values, not sum)
  ...
  ################  partition specific model on test ################
  ...
  for(i in 1:length(unique_interaction_vars)) {
    ...
    for(e in unique_environments) {
      ...
      # Run lasso 
      lasso.fit <- big_spLinReg(G.real.subset.env, y=y[index_e], dfmax=dfmax_lasso_all, ncores=ncores, 
...
results.df = data.frame(fdp = fdp, 
                        ...
                        sparsity = sparsity, amp = amp, fdr = fdr, propmain = propmain, method = "naive")
...
# Save res 
stats.path = paste0("UKB_stats/simint_paper_naive_lasso_",
