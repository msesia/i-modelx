...
N_LIST=(1000)
P_LIST=(100)
SIGNAL_MEAN_LIST=(200 300 400 500)
SIGNAL_STD_LIST=(100)
BATCH_LIST=$(seq 1 10)
BATCH_SIZE=1
    ...
    for SIGNAL_MEAN in "${SIGNAL_MEAN_LIST[@]}"; do
      for SIGNAL_STD in "${SIGNAL_STD_LIST[@]}"; do
        for BATCH in $BATCH_LIST; do
          ...
          JOBN="n"$N"_p"$P"_a"$SIGNAL_MEAN"-"$SIGNAL_STD"_b"$BATCH

          OUT_FILE=$OUT_DIR"/estimation_"$JOBN".txt"
          ...
          if [[ $RUN == 1 ]]; then
            # Script to be run
            SCRIPT="experiment.sh $N $P $SIGNAL_MEAN $SIGNAL_STD $BATCH $BATCH_SIZE"
