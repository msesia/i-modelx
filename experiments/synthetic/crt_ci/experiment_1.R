...
treat.threshold = as.numeric(args[6]) / 100
tail <- as.character(args[7])
alpha = as.numeric(args[8]) / 100
...
out.file = sprintf("%s/n%d_p%d_a%d-%d_i%d_t%s_%s_alpha%d_k%d.txt",
                   out.dir, n, p, round(signal_mean*100), round(signal_std*100), num_inter, round(100*treat.threshold),
                   tail, round(100*alpha), k.rel)
    ...
    for(multivariate in c(FALSE, TRUE)) {
        for(reference in c("treated", "control", "auto")) {
            for(j.rel in 1:p1) {
                ...
                j <- idx_treat[j.rel]
                tmp <- check_hypothesis(beta, j, k, 0, tail, verbose=TRUE)

                start_p = Sys.time()

                bound.value = experiment(j.rel, seed, alpha=alpha, multivariate=multivariate, reference=reference, bound=bound)

                end_p = Sys.time()
                print(end_p-start_p)

                cat(sprintf("Completed experiment %d of %d with seed %d.\n", b, batch.size, seed))
                res = tibble(j=j.rel, seed=seed, bound.value=bound.value, tau_k=tmp$tau_k, multivariate=multivariate, reference=reference, null=tmp$is_null)
                flush.console()
