...
crt.loop <- function(j, X, prob_treat, y, beta.lower, beta.upper, beta0.grid, lamb.precomputed,
    ...
    compute.importance.glmnet.inter <- function(X_tmp, y.bar, z, beta0, lamb=NULL, base.model=NULL) {
        X_tmp[,j] <- z
        data <- as.data.frame(cbind(y.bar, X_tmp))
        ...
        if(is.null(base.model)) {
            ...
            opt.lasso <- glmnet::cv.glmnet(x, y.tmp, family="gaussian")
            beta.hat.vec <- coef(opt.lasso, s="lambda.min")[-1]
            names(beta.hat.vec) <- x.names
            idx <- sapply(1:length(x.names), function(i) grepl(paste(":",x.names[j],sep=""), x.names[i], fixed = TRUE))
            beta.hat.vec.j <- beta.hat.vec[c(j,which(idx))]
            beta.hat <- sum(abs(beta.hat.vec.j))
        } else {
            idx.active <- which(!is_inactive(prob_treat))
            pred <- as.numeric(x[idx.active,] %*% base.model)
            residuals <- y.tmp[idx.active]-pred
            beta.hat <- as.numeric(coef(lm(residuals~z[idx.active]))[2])
            beta.hat <- ifelse(is.na(beta.hat), 0, beta.hat)
        }
