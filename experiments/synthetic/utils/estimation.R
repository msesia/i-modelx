...
adjust_Y = function(Y, Z, Z_new, delta, k, side="right") {
...
compute_pval = function(X, Y, j, prob_treat, k, delta, multivariate=FALSE, tail="right", reference="auto") {
...
interval.step = function(Y_in, X, j, bound, alpha=0.1, c=1, step=1,
    ...
    if(multivariate) {
        # Compute residuals
        mod = cv.glmnet(X[,-j],Y_in)
        Y = Y_in - predict(mod, X[,-j], s="lambda.min")
    } else {
        Y = Y_in
    }
    ...
    if(tail=="right") {
        if(T > T_new) { ## Most common scenario (ideally happening with prob 1-alpha)
            ## Cautiously move up
            move <- c*(alpha)/(1+step/100)
        } else {
            ## Move back down
            move <- -c*(1-alpha)/(1+step/100)
        }
    } else {
         if(T > T_new) { ## Most common scenario (ideally happening with prob 1-alpha)
            ## Cautiously move up
    ...
    if(verbose) {
        cat(sprintf("Bound: %.3f, %.3f. Delta = %.3f\n", T, T_new, move))
    }
...
compute_interval = function(X, Y_in, j, prob_treat, k, multivariate=FALSE, bound="lower", reference="auto",
                            n.steps = 1000, verbose=FALSE) {
    ...
    for(s in 1:n.steps) {
        step = step+1
        delta = interval.step(Y, X, j, delta, tail=tail, multivariate=multivariate, verbose=verbose)
    ...
    ## Combine results
    out <- c()
    out$bound <- mean(tail(history, 100)$bound)
    out$history <- history
    out$delta_k <- delta_k
