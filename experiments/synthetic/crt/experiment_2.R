...
treat.threshold = as.numeric(args[5]) / 100
tail <- as.character(args[6])
delta = as.numeric(args[7]) / 100
...
header = tibble(n=n, p=p, p_causal=p_causal, p_causal_covar=p_causal_covar,
                signal_mean=signal_mean, signal_std=signal_std, prop.treat=prop.treat, rho=rho,
...
## Output file
out.dir = "results/crt_2"
out.file = sprintf("%s/n%d_p%d_a%d-%d_t%d_%s_%d_k%d.txt",
                   out.dir, n, p, round(signal_mean*100), round(signal_std*100), round(100*treat.threshold), tail, round(100*delta), k.rel)
